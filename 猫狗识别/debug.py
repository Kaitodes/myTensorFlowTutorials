...
#           self.__save_image(links, word)
    ...
    def __save_image(self, links, word):
        ...
        for link in links:
            try:
                ...
                urllib.request.urlretrieve(link, './' + word + '/%s.jpg' % (self.__counter - 1))
    ...
    def start(self, word, spider_image_num, start_image):
        ...
        """
        print("********************************************************")
        print("从Bing爬取题为：" + word + "图片，"+"从第" + str(start_image) + "张开始获取," + "一共获取" + str(spider_image_num) + "张")
        print("********************************************************")
...
#    print (args.number, args.first)
    crawler = Crawler(0.05)
    crawler.start(args.keywords, args.number, args.first)
